...
:::
::: {.column width="46%"}
- **Benchmarks:** MATH-500, AIME, GPQA Diamond, SWE-bench.
...
**Group Relative Policy Optimization** (DeepSeek) is the *de facto* RLVR algorithm.
...
::: {.column width="44%"}
::: {.card}
"Beat your own average." No separate **value network** (unlike PPO) — the group's mean *is* the baseline. Cheaper, simpler, stable.
:::
:::
:::
...
::: {.columns}
::: {.column width="52%"}
- **KL penalty** — stay close to the base model, so it doesn't forget language or drift into gibberish.
- **Ratio clipping** (from PPO) — cap how far one update can move the policy → no wild swings.
- **Format reward** — a bonus for wrapping thoughts in `<think>…</think>` and the answer in `\boxed{}`.
:::
::: {.column width="48%"}
::: {.card}
Total reward = **correctness** + **format**.
Loss ≈ *advantage-weighted log-prob* − *KL to base*.

The format reward makes the output **parseable** by the verifier and cleanly **separates thinking from answer**.
:::
:::
:::
...
|:--|:--|:--|
| **OpenAI** | GPT-5.x, o-series | coding, tool use |
| **Google** | Gemini 3.x Pro (thinking) | leads many pure-reasoning benchmarks |
| **Anthropic** | Claude Opus 4.x (extended thinking) | long agentic coding runs |
| **DeepSeek / Qwen / GLM** | R-series, Qwen, GLM (open) | strong & **cheap**, open weights |

::: {.card}
Benchmarks like **GPQA Diamond** (hard science) and **SWE-bench** (real GitHub bugs) now sit around **80–95%** — territory that was science-fiction in 2022.
:::
