...
::: {.card}
~10 years: ML went from academic curiosity → the tech behind search, recommendations, translation, and ChatGPT.
:::
...
::: {.card}
Classic ML: one dataset → one model → one task. LLM: one giant pre-trained model → **many** tasks.
:::
...
## The story didn't stop in 2022 {.smaller}

Since ChatGPT, the field has moved fast. A few directions shaping **2023–2026**:
...
::: {.src}
Frontier examples as of **mid-2026** — model names change fast; check the latest before class.
:::
...
:::
::: {.column width="50%"}
- **Tools** — search, run code, call APIs, click buttons.
- **Memory** — carry context across steps.
- **Open standard** — **MCP** (now Linux-Foundation–governed, 10k+ connectors) lets any tool plug into any model.

**Example (2026):** *OpenClaw* 🦞 — open-source assistant that runs on your own machine and orchestrates tasks across email, calendar & chat apps.
:::
:::
...
::: {.card}
A **multimodal LLM** (GPT-4o, Gemini) can look at an image and answer in words — tying vision back to §5–§6.
:::
:::
::: {.column width="40%"}
...
- **Public cloud ML:** SageMaker, Vertex AI, Azure ML — managed GPU + endpoints.
