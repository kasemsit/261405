...
::: {.notes}
แนวทางเฉลย (ไม่มีคำตอบเดียวตายตัว — เน้นกระบวนการคิด framing):

**1) Business vs ML objective**
- *Business*: เพิ่ม engagement / retention — ช่วยให้ผู้ใช้เห็น "สิ่งที่กำลังเกิดตอนนี้" อยู่กับแพลตฟอร์มนานขึ้น
- *ML*: ต้องแปลงเป็นเป้าที่ "วัดได้" — เช่น ให้คะแนน *ความกำลังมา (trendiness)* ของแต่ละแฮชแท็ก ณ ตอนนี้ แล้วเอามาจัดอันดับ
- จุดสำคัญ (โยง §9 proxy metric): **"trending" ≠ "ใช้เยอะสุด"** — #love ถูกใช้เยอะทุกวันแต่ไม่ได้กำลังมา · trending คือ *การพุ่งผิดปกติเทียบกับ baseline* (spike / anomaly / velocity) ถ้า optimize ยอดดิบเฉย ๆ จะได้สแปม/บ็อต/การปั่น

**2) Classification, regression, หรือ ranking?**
- กรอบที่ดีสุดคือ **ranking** — จัดอันดับแฮชแท็กแล้วโชว์ top-K
- สัญญาณข้างใต้เป็น **regression** ได้ — ทำนายคะแนนตัวเลข เช่น อัตราเร่งของการใช้งาน (rate of change) ต่อแฮชแท็ก แล้วเรียงตามนั้น
- *ไม่ใช่* classification (เส้นแบ่ง trending/ไม่ trending เป็น threshold ที่ตั้งเอง) — โยงกลับสไลด์ "Framing" ว่าโจทย์เดียวมองได้หลายแบบ

**3) วัดผล / มี ground truth ไหม?**
- ตัวยาก: **ไม่มี label "trending" ที่เป็นความจริงสำเร็จรูป** — ต้องสร้างนิยามเอง
- *Online (ตัววัดจริง)*: A/B test — CTR ของโมดูล trending, dwell time, sessions, retention ปลายทาง
- *Offline proxy*: backtest — คะแนนที่ทำนายวันนี้ "มาก่อน" ยอดพุ่งจริงในอนาคต (t+1) ไหม
- *Human / guardrails*: คนรีวิวความเกี่ยวข้อง + กรองการปั่นจากบ็อต, สแปม, NSFW, misinfo (โยง §9)

ประเด็นที่อยากให้นักศึกษาได้: **business objective → ML objective ต้องเลือก proxy ดี ๆ, โจทย์เดียว frame ได้หลายแบบ, และบางงานไม่มี ground truth สำเร็จรูปต้องออกแบบการวัดเอง**
:::
...
::: {.src}
Mid-2026: the **harness** often matters as much as the model — the same LLM under a better harness can swing solve rates by *tens of points* on benchmarks like SWE-bench ([SWE-agent](https://arxiv.org/abs/2405.15793); [Harness-Bench](https://arxiv.org/abs/2605.27922)). MCP is now an open standard (Agentic AI Foundation); learn the **loop**, not the product.
:::

## What is a *harness*? {.smaller}
...
![](images/harness.svg){.diagram width="72%"}
...
::: {.column width="45%"}
::: {.card .blue}
Same model, better harness → big gains: [SWE-agent](https://arxiv.org/abs/2405.15793) · [Harness-Bench](https://arxiv.org/abs/2605.27922)
:::
:::
:::
...
## Your task — brainstorm (15 min, groups of 4–5) {.smaller background-color="#f4c542"}
...
![](images/icu-pipeline.svg){.diagram width="96%"}
...
## References — agents, vision & responsible AI {.smaller}

**Agents & the harness**

- Yao et al. (2022), [*ReAct: Reasoning + Acting*](https://arxiv.org/abs/2210.03629) · Yang et al. (2024), [*SWE-agent*](https://arxiv.org/abs/2405.15793) · [*Harness-Bench*](https://arxiv.org/abs/2605.27922) (2026)
- Anthropic, [*Building Effective Agents*](https://www.anthropic.com/engineering/building-effective-agents) · [Model Context Protocol](https://modelcontextprotocol.io)
...
- Ribeiro et al. (2016), [*LIME*](https://arxiv.org/abs/1602.04938) · Lundberg & Lee (2017), [*SHAP*](https://arxiv.org/abs/1705.07874) — interpretability
- Mitchell et al. (2019), [*Model Cards*](https://arxiv.org/abs/1810.03993) · Gebru et al. (2021), [*Datasheets for Datasets*](https://arxiv.org/abs/1803.09010)
- Strubell et al. (2019), [*Energy & Policy Considerations*](https://arxiv.org/abs/1906.02243) — green AI · NIST, [*AI Risk Management Framework*](https://www.nist.gov/itl/ai-risk-management-framework)
- *Getty Images (US) Inc v Stability AI Ltd* [2025] EWHC 2863 (Ch) — training-data copyright
